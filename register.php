...
if ($result->num_rows == 0) {
    // Insert statement
   // $query_stry = "INSERT INTO users VALUES (NULL, '$user_name', '$full_name', '$user_email', '$password', '$role', '$profilePicturePath')"; // prbolem as user id is null so useraccount.php is not showing 
...
if ($conn->query($query_stry) === TRUE) {
    ...
    // $result = $conn->query($query_stry); 2nd time user gets create 
} else {
    echo "Error: " . $query_stry . "<br>" . $conn->error;
}
//$insert_result = @$conn->query($query_stry); 3rd time user gets create // remove query and install query_stry
    ...
    $_SESSION['id'] = $result_row['user_id']; //  here user id is null 
